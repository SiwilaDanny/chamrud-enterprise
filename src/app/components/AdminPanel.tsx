                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="col-span-2">
                      <label className="text-xs font-semibold text-muted-foreground uppercase mb-1 flex items-center justify-between">
                        <span>Image URL</span>
                        <label className={`cursor-pointer flex items-center gap-1 text-[#149CD8] hover:text-[#003399] transition-colors ${isUploading ? 'opacity-50 pointer-events-none' : ''}`}>
                          <Upload className="w-3 h-3" />
                          {isUploading ? 'Uploading...' : 'Upload File'}
                          <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, "product")} />
                        </label>
                      </label>
                      <input type="url" value={productForm.image || ""} onChange={e => setProductForm(f => ({ ...f, image: e.target.value }))} className="w-full px-3 py-2 rounded-lg border bg-white text-sm focus:border-[#003399] outline-none" />
                    </div>
                  </div>
                </div>
                <div className="px-6 py-4 border-t border-border bg-white flex gap-3 flex-shrink-0">
